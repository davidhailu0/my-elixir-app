defmodule MyLiveAppWeb.CounterLive.FormComponent do
  use MyLiveAppWeb, :live_component
  ...
  def update(%{counter: counter} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Pages.change_counter(counter))
     end)}
  end

  @impl true
  def handle_event("validate", %{"counter" => counter_params}, socket) do
    changeset = Pages.change_counter(socket.assigns.counter, counter_params)
  ...
  defp save_counter(socket, :edit, counter_params) do
    case Pages.update_counter(socket.assigns.counter, counter_params) do
      {:ok, counter} ->
  ...
  defp save_counter(socket, :new, counter_params) do
    case Pages.create_counter(counter_params) do
      {:ok, counter} ->
        notify_parent({:saved, counter})
